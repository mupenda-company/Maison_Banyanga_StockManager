...
class MissionController extends Controller
{
    ...
    public function createRestourne()
    {
        ...
        $ristournes = $this->db->fetchAll(
            "SELECT r.*, c.nom as client_nom, c.numero_client, c.zone_id, z.nom as zone_nom
             FROM ristournes r
    ...
    public function terminer($id)
    {
        ...
        if ($result['success']) {
            ...
            if (is_array($billetage)) {
                $billetageModel = new Billetage();
                if ($billetageModel->totalBase($billetage) > 0) {
                    $billetageModel->saveForReference('mission', (int) $id, $billetage, $_SESSION['user_id'] ?? null);
                }
            }
            return $this->success(null, 'Mission terminee et stock mis a jour avec succes');
        }
