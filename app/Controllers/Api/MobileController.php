...
class MobileController extends Controller {
    ...
    public function getStock($missionId) {
        ...
        $mission = $this->db->fetch(
            ...
            "SELECT id, statut, COALESCE(type_mission, 'vente') as type_mission
        ...
        $usageStock = ($_GET['usage'] ?? '') === 'ristourne' ? 'ristourne' : 'vente';
        ...
        $sql = "SELECT p.id, p.nom, p.code, p.bouteilles_par_caisses,
                ...
                WHERE mc.mission_id = ?
                  ...
                  AND COALESCE(mc.type_chargement, 'vente') = ?";

        $stock = $this->db->fetchAll($sql, [$missionId, $usageStock]);
...

    {
        ...
        $stockUsageForStats = (($mission['type_mission'] ?? 'vente') === 'ristourne') ? 'ristourne' : 'vente';
        $chargementTotals = $this->db->fetch(
            "SELECT
             ...
             WHERE mc.mission_id = :mission_id
               AND COALESCE(mc.quantite_caisses, 0) > 0
               AND COALESCE(mc.type_chargement, 'vente') = :type_chargement",
            ['mission_id' => $missionId, 'type_chargement' => $stockUsageForStats]
        );
        ...
        return $this->success([
            ...
            'produits_mission' => $this->db->fetchAll(
                "SELECT p.id, p.nom, p.code, p.prix_vente_caisses, p.prix_vente_unitaire, p.bouteilles_par_caisses,
                        mc.quantite_caisses,
                        IFNULL(mc.quantite_vendue, 0) as quantite_vendue,
                        (IFNULL(mc.quantite_caisses, 0) - FLOOR(IFNULL(mc.quantite_vendue, 0) / COALESCE(NULLIF(p.bouteilles_par_caisses, 0), 24))) as stock_actuel_caisses
                 FROM mission_chargements mc
                 JOIN produits p ON mc.produit_id = p.id
                 WHERE mc.mission_id = :mission_id
                   AND COALESCE(mc.quantite_caisses, 0) > 0
            ...
            'stock' => [
               ...
               AND COALESCE(mc.quantite_caisses, 0) > 0
               AND COALESCE(mc.type_chargement, 'vente') = :type_chargement", ['mission_id' => $missionId, 'type_chargement' => $stockUsageForStats]))),
...

    {
        ...
        $produitId = (int) ($data['produit_id'] ?? $row['produit_id'] ?? 0);
        ...
        if (!$chargement) {
            return $this->error('Ce produit ne fait pas partie du chargement de la mission', 422);
        }

        $bouteillesParCaisse = (int) ($chargement['bouteilles_par_caisses'] ?? 24);
        if ($bouteillesParCaisse <= 0) $bouteillesParCaisse = 24;
        $prixCaisse = (float) ($chargement['prix_vente_caisses'] ?: (($chargement['prix_vente_unitaire'] ?? 0) * $bouteillesParCaisse));
        if ($prixCaisse <= 0) {
            return $this->error('Prix caisse invalide pour le produit choisi', 422);
        }
        ...
        $stockRestantBouteilles = ((float) ($chargement['quantite_caisses'] ?? 0) * $bouteillesParCaisse) - (float) ($chargement['quantite_vendue'] ?? 0);

        if ($caissesLivrees * $bouteillesParCaisse > $stockRestantBouteilles) {
    ...
    public function storeVente() {
        ...
        try {
            ...
            $emballagesRecus = $this->normaliserEmballagesRecusMobile($data['emballages_recus'] ?? null, $data['produits'] ?? []);

            $totalCaissesVendues = 0;

            $videsRestantsAAffecter = array_sum($emballagesRecus);
            ...
            foreach ($data['produits'] as $item) {
                ...
                $details[] = [
                    ...
                    'caisses_vides_recues' => $caissesVidesAffectees,
            ...
            if (array_sum($emballagesRecus) > $totalCaissesVendues) {
            ...
            foreach ($details as $detail) {
                ...
                $this->db->query(
                    ...
                    "UPDATE mission_chargements
                     ...
                     WHERE mission_id = :m AND produit_id = :p AND COALESCE(type_chargement, 'vente') = 'vente'",
            ...
            foreach ($emballagesRecus as $produitId => $caissesRecues) {
                ...
                $quantiteVides = $caissesRecues * $bouteillesParCaisse;
    ...
    private function normaliserEmballagesRecusMobile($emballagesRecus, array $produits)
    {
        $result = [];
        ...
        foreach ($produits as $ligne) {
            ...
            $produitId = (int) ($ligne['produit_id'] ?? 0);
            $caisses = max(0, (int) ($ligne['caisses_vides_recues'] ?? 0));
            if ($produitId > 0 && $caisses > 0) {
                $result[$produitId] = ($result[$produitId] ?? 0) + $caisses;
            }
        }
