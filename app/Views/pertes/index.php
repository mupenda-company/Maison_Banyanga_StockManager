...
<div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6 no-print">
    ...
    <?php if (can('pertes.imprimer')): ?><button type="button" onclick="window.open('<?= htmlspecialchars($printUrl, ENT_QUOTES, 'UTF-8') ?>','_blank')" class="btn btn-secondary">Imprimer</button><?php endif; ?>
    <?php if (can('pertes.exporter')): ?><a href="<?= htmlspecialchars($exportUrl, ENT_QUOTES, 'UTF-8') ?>" class="btn btn-secondary">Exporter Excel</a><?php endif; ?>
                    ...
                    <tr>
                        ...
                        <td class="text-center whitespace-nowrap">
                            <div class="flex items-center justify-center gap-2">
                                <?php if (can('pertes.modifier')): ?>
                                <a href="<?= url('pertes/' . (int)$perte['id'] . '/edit') ?>" class="text-primary-600 hover:text-primary-800" title="Modifier">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                    </svg>
                                </a>
                                <?php endif; ?>
                                <?php if (can('pertes.supprimer')): ?>
                                <button onclick="supprimerPerte(<?= (int)$perte['id'] ?>)" class="text-red-600 hover:text-red-800" title="Supprimer">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
